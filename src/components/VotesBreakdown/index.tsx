        ...
        <div className="flex flex-col gap-2">
          {votesBreakdown
            ...
            .map((vote) => {
              ...
              return (
                <div key={vote.role} className="flex justify-between items-center">
                  <div
                  ...
                  >
                    {type !== 'BOTH' &&
                      (type === 'FOR' ? `+ ${vote.votes_for}` : `- ${vote.votes_against}`)}
                  </span>
                  {type === 'BOTH' && (
                    <span className="font-bold text-xs text-muted-foreground">
                      <span className="text-emerald-400">+ {vote.votes_for}</span> /{' '}
                      <span className="text-red-400">- {vote.votes_against}</span>
                    </span>
                  )}
                </div>
              );
